...
public final class JavaFileObjectUtils {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JavaFileObjectUtils.class);

    private JavaFileObjectUtils() {
    }

    /**
     * Creates a {@link JavaFileObject} with a path corresponding to the
     * {@code fullyQualifiedName} containing the give {@code source}. The
     * returned object will always be read-only and have the
     * {@link Kind#SOURCE} {@linkplain JavaFileObject#getKind() kind}.
     *
     * <p>
     * Note that this method makes no attempt to verify that the name matches
     * the contents of the source and compilation errors may result if they do
     * not match.
     */
    ...
    /**
     * Behaves exactly like {@link #forSourceString}, but joins lines so that
     * multi-line source strings may omit the newline characters. For example:
     * <pre>   {@code
     *
    ...
    /**
     * An overload of {@code #forSourceLines} that takes an
    ...
    public static Kind deduceKind(URI uri) {
        ...
        logger.debug("[JavaFileObjectUtils => deduceKind] from URI path:'{}' deducedKind is:'{}'",path,deducedKind);
        return deducedKind;
    }
    ...
    private static final class JarFileJavaFileObject extends ForwardingJavaFileObject<ResourceSourceJavaFileObject> {
